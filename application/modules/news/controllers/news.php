...
class News extends MX_Controller {
    ...
    public function index() {
        if ($this->session->userdata('admin_id') == null) {
            ...
            $config['total_rows'] = $this->news_model->total();
            $config['per_page'] = 5;
            $config['num_links'] = 10;
            ...
            if (isset($_REQUEST['btt_submit_search'])) {
                ...
                $data['list_news'] = $this->news_model->search_keyword($keyword, $config['per_page'], $this->uri->segment(2));
            } else {
                $data['list_news'] = $this->news_model->list_all_by_page($config['per_page'], $this->uri->segment(3));
            }
    ...
    public function create_new() {
        if ($this->session->userdata('admin_id') == null) {
            ...
            if (isset($_REQUEST['btt_submit'])) {
                ...
                $newsid = $this->news_model->insert($params);
                ...
                for ($i = 0; $i < $count; $i++) {
                    $this->news_model->updateimagepro($imageid[$i], $newsid);
                }
                redirect('news/index');
            }
            $data['list_category'] = $this->categories_model->list_all();
    ...
    public function update($id = null) {
        if ($this->session->userdata('admin_id') == null) {
            ...
            $data['allimages'] = $this->news_model->list_image();
            $data['imageid'] = $this->news_model->return_imageid($id);
            ...
            if (isset($_REQUEST['btt_submit'])) {
                ...
                $count = count($this->input->post('file_image'));
                $exit = $this->input->post('file_exit');


                for ($i = 0; $i < $count; $i++) {
                    if ($imageid) {
                        $this->news_model->updateimagepro($imageid[$i], $id);
                    }
                }
